...
namespace ContosoUniversity.Controllers
{
    public class CoursesController : BaseController
    {
        ...
        public IActionResult Create([Bind("CourseID,Title,Credits,DepartmentID,TeachingMaterialImagePath")] Course course, IFormFile teachingMaterialImage)
        {
            if (ModelState.IsValid)
            {
                if (teachingMaterialImage != null && teachingMaterialImage.Length > 0)
                {
                    ...
                    if (!System.Array.Exists(allowedExtensions, e => e == fileExtension))
                    {
            ...
            if (ModelState.IsValid)
            {
                if (teachingMaterialImage != null && teachingMaterialImage.Length > 0)
                {
                    ...
                    try
                    {
                        ...
                        {
                            var relPath = course.TeachingMaterialImagePath.TrimStart('~', '/').Replace('/', Path.DirectorySeparatorChar);
                            var oldFilePath = Path.Combine(_env.ContentRootPath, relPath);
        ...
        public IActionResult DeleteConfirmed(int id)
        {
            ...
            {
                var relPath = course.TeachingMaterialImagePath.TrimStart('~', '/').Replace('/', Path.DirectorySeparatorChar);
                var filePath = Path.Combine(_env.ContentRootPath, relPath);
